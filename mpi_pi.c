...
int main ( int argc , char *argv[ ] )
{
  ...
  MPI_Init(&argc,&argv);
  MPI_Comm_size(MPI_COMM_WORLD,&numprocs);   // &numprocs = #of_processors_used
  MPI_Comm_rank(MPI_COMM_WORLD,&myid);       // &myid 0 id of the current processor
...
// take time of processors after initial I/O operation
  ...
  if (myid ==0) { //if I am the master process gather results from others
    ...
    for (proc=1; proc<numprocs ; proc++) {
      MPI_Recv(&local_M,1,MPI_LONG_LONG,proc,tag,MPI_COMM_WORLD,&status ) ;  // Message passing from slaves to master (in the way it's implemented, it's low efficient)
