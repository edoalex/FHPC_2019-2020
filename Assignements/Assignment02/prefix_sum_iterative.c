...
#pragma omp critical
  ...
  //---------------------------------  
   ...
   int** pyramid;
   if ( (pyramid = (int**)malloc( (max_split + 1) * sizeof(int*) )) == NULL ){
     printf("I'm sorry, on some thread there is not enough memory to host %lu bytes\n\n", (max_split + 1) * sizeof(int));
     return 1;
   }

    if ( (level_lenght = (int*)malloc( (max_split + 1) * sizeof(int) )) == NULL ){
     printf("I'm sorry, on some thread there is not enough memory to host %lu bytes\n\n", (max_split + 1) * sizeof(int));
     return 1;
   }

    for(unsigned int level=0; level <= max_split; level++){
     if ( (pyramid[level] = (int*)malloc( N/ pow(2,level) * sizeof(int) )) == NULL ){
     printf("I'm sorry, on some thread there is not enough memory to host %f bytes\n\n", N/ pow(2,level) * sizeof(int));
     return 1;
     }
    }
    
    // u can merge the 2 for ehre EDOOO
    
   for(unsigned int level=0; level <= max_split; level++)
     level_lenght[level] = N / pow(2,level);
  ...
  // ------------ first part: going up the pyramid
     ...
     for(unsigned int level=1; level <= max_split; ++level){
       ...
       #pragma omp parallel for
       for(unsigned int i=0; i<level_lenght[level]; i++)
  ...
  // ------------ second part: going down the pyramid, filling the output array level by level
     ...
     for(unsigned int i=1; i<level_lenght[max_split]; ++i)
...
#pragma omp parallel for shared(y,pyramid)
      for(unsigned int i=1; i<level_lenght[level]; i += 2)
